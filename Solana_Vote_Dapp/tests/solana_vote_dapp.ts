...
const expectAnchorErrorCode = (err: unknown, expectedCode: string) => {
  ...
  const actualCode = anyErr?.error?.errorCode?.code??
...
describe("3. Voter", () => {
  it("3.1  register voter", async () => {
    ...
    console.log("voter account data", voterAccountData);
...
describe("4.Proposal Registration", () => {
  it("4.1  register proposal", async () => {
    ...
    const proposalInfo = "Vote for the good candidates";
    const stakeTokenAmount = new anchor.BN(1000);

    await program.methods.registerProposal(proposalInfo, deadlineTime, stakeTokenAmount).accounts({
    ...
    expect(proposalAccountData.proposalInfo).to.equal(proposalInfo);
...
describe("5.Casting Vote", () => {
  it("5.1 casts vote", async () => {
    ...
    await program.methods.proposalToVote(PROPOSAL_ID, stakeTokenAmount).accounts({
      authority: voterWallet.publicKey,
...
describe("6.Pick Winner", () => {
  it("6.1 Should Fail to pick winner before deadline passes", async () => {
    ...
    it("6.2 Should pick winner after deadline passes", async () => {
      ...
      // Wait for 12 seconds to ensure deadline has passed
...
describe("9.SOL Withdrawl", () => {
  it("8.1 Should allow admin to withdraw SOL from treasury", async () => {
   ...
   //Fist check if there's enough SOL in the vault
   ...
   if(vaultBalance >= withdrawAmount.toNumber()){
    ...
    console.log("Not enough SOL in vault to withdraw");
   }
  });
