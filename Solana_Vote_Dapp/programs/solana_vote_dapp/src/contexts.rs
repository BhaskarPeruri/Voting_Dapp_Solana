...
pub struct InitializeTreasury<'info> {
    ...
    #[account(init, payer = authority, mint::decimals = 6, mint::authority = mint_authority, seeds = [b"x_mint"], bump)]
    pub x_mint: Account<'info, Mint>,

    #[account(init, payer = authority, associated_token::mint=x_mint, associated_token::authority = authority)]
    pub treasury_token_account: Account<'info, TokenAccount>,

    #[account(init, payer = authority, space = 8 + ProposalCounter::INIT_SPACE, seeds  = [b"proposal_counter"], bump)]
...
pub struct RegisterProposal<'info> {
    ...
    #[account(init, payer = authority, space = 8 + Proposal::INIT_SPACE, seeds  = [b"proposal", proposal_counter_account.proposal_count.to_le_bytes().as_ref()], bump)]
    ...
    #[account(mut, constraint= proposal_token_account.mint == x_mint.key()
...
pub struct Vote<'info> {
   #[account(mut, seeds = [b"voter", authority.key().as_ref()], bump,
constraint = voter_account.proposal_voted == 0)]//one voter one vote

pub voter_account:Account<'info, Voter>,
pub x_mint: Account<'info, Mint>,

#[account(mut, constraint = voter_token_account.mint ==x_mint.key() && voter_token_account.owner == authority.key())]
...
pub struct CloseProposal<'info>{
    #[account(
        ...
        bump,
        close = destination,
        constraint = proposal_account.authority == authority.key() @VotingErrorCode::UnAuthorized
    )]
    pub proposal_account: Account<'info, Proposal>,
...
pub struct WithdrawSol <'info>{
    #[account(
        ...
        bump,
        constraint = treasury_config.authority == authority.key() @VotingErrorCode::UnAuthorized
    )]
